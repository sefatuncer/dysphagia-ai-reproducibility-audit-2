...
| # | Item | Status | Where |
|---|---|---|---|
| B0 | Immutable state: the observed state of each audited artifact is fixed by an identifier | **Met, retrospectively** | Not recorded at intake. Recovered afterwards by `scripts/25_commit_provenance.py` for 22 of 23 rows; the exception is the empty repository, which has no commit to record and whose emptiness the same call corroborates (HTTP 409, 0 KB, creation and last push one second apart). `scripts/26_external_deposit_provenance.py` records the depositor's MD5 for the 6.1 GB weight archive. *That checksum is the depositor's, not a hash of the copy we ran, which was deleted; bit-identity with our copy is not demonstrable and we do not claim it.* The manuscript states both the recovery and its limit. |
| B0b | Measurement error of the audit itself is quantified | **Met** | `scripts/27_intake_reverification.py` recomputes every intake signal from the tree at each recovered commit. All six signals reproduced in all 20 repositories the script coded, 0 disagreements. The 4 disagreements are in the 2 pilots entered by hand before the scripted intake and are notation differences or the data-directory rule limitation already corrected and reported. |
| B1 | Inclusion rule stated before the reader sees the results | **Met** | Manuscript, Eligibility and Discovery; `protocol/protocol.md`. No registry entry was made, and the manuscript gives the reason rather than implying one exists. *Correction (v1.1.0): `transparency/statistical-analysis-plan.md` described itself as "locked before any outcome data were seen". The manuscript claims the opposite — the protocol is a dated record with no claim of temporal priority — so the archive was asserting a priority the study does not have. The line is corrected and the previous wording is quoted in place rather than deleted.* |
...
| B5 | Verdict definitions stated with explicit criteria | **Met, with a qualification** | Manuscript, Containerized Reruns, with four mutually exclusive verdicts with explicit criteria. The harness entry condition was written after the three builds, and the article dates it, so the definitions are not claimed to predate the builds. |
...
| # | Item | Status | Where |
|---|---|---|---|
| C1 | Denominators stated explicitly; "not reported" never imputed | **Met** | Manuscript, Statistical Analysis. |
| C2 | Interval estimates interpreted in line with the sampling design | **Met** | Wilson intervals reported as descriptive indicators of imprecision, not as inference to the wider literature. |
