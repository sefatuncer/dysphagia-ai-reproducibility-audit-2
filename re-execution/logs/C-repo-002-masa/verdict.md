...
[Corrected 2026-09-17: commit identifiers were later recovered for every audited repository except the empty one, from the GitHub commits API at each logged access date (`scripts/25_commit_provenance.py`), and the intake signals were re-read at those commits (`scripts/27_intake_reverification.py`). The paragraph above predates that step and is kept as written. For this repository the recovered commit is the one current on 14 July 2026, dated 12 July 2026; this log also gives 11 July 2026 as the access date, so the state that was built cannot be fixed to one commit with certainty. The build failure recorded here turns on the `pocketsphinx` entry pinned to a local Windows wheel, and both the recovered commit and the commit current on 11 July 2026 (`2bd77dc`) carry it, at line 23 of `requirements.txt` in each; the line number 30 given above does not match either and is kept as originally written.]
