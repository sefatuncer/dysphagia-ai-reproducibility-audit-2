...
> deficiency code; RS5 now describes recruitment and setting. (3) Terminology: the article
> avoids the term "clinical reproducibility" used below and keeps label-measurement
> reliability (RS4) distinct from validity (the other five items). The clinician author
> reviewed and accepted these changes on 17 September 2026; no code was changed by them.
...
3. **The predicted target is often not the instrumental standard** `[NKT]`: one model substitutes a **surrogate** — *B* predicts a bedside screening label from voice rather than the instrumental standard itself. Two predict a **different target type** rather than a weaker surrogate of the instrumental scale — *Q* (tube feeding and pneumonia) and *R* (CTCAE toxicity). *O* belongs to neither group and is listed under instrumental reference above: its target is VFSS-confirmed dysphagia, and the clinical and voice features are inputs. One model carries the **same signal** code — *A*, whose 23 input parameters are measured from the recordings two experts scored to produce the label.

**The criterion for the same signal code, since it would otherwise fit almost every imaging row.** Read literally the code would apply to *D*, *E*, *G* and *M*, whose masks are drawn on the images being segmented, and to *N*, whose annotation lies on the manometry trace the model consumes. Those are measurement tasks, appraised as agreement with expert annotation, and on them the code merely repeats what their own description already says. It is applied only where the endpoint is a **diagnostic claim**, because the accuracy reported there is agreement with the raters who read the same recording rather than accuracy against an independently obtained standard. *A* is the one row where that holds, and it holds with no standardized scale and no reported rater agreement.
