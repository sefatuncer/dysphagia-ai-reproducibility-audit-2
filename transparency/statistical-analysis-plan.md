...
## 4. Reliability (κ)
- **Screening κ:** Cohen κ for each **independent** pair (S.T. ↔ methodologist; N.K.T. ↔ methodologist) rather than for the two-author pair, which is not independent. **A 95% interval (bootstrap, `03_analysis.py:cohen_kappa_ci`, fixed seed) plus the Landis–Koch interpretation.** *(Not executed: no methodologist joined the study. What was done instead is the blind rule-based re-coding reported in the manuscript.)*
